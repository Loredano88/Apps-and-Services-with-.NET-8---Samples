...
namespace Northwind.Common.EntityModels.Tests
{
    public class NorthwindEntityModelsTests
    {
        ...
        public void ProductId1IsChai()
        {
            using(NorthwindContext db = new())
            {
                Product product1 = db.Products.Single(_ => _.ProductId == 1);
        ...
        public void EmployeeHasLastRefreshedIn10sWindows()
        {
            using(NorthwindContext db = new())
            {
                Employee employee1 = db.Employees.Single(_ => _.EmployeeId == 1);
                DateTimeOffset now = DateTimeOffset.UtcNow;
                Assert.InRange(actual: employee1.LastRefreshed, low: now.Subtract(TimeSpan.FromSeconds(5)), high: now.Add(TimeSpan.FromSeconds(5)));
            }
        }
    }
}
...
partial class Program
{
    ...
    private static void OutputCasing(string original)
    {
    ...
    private static void OutputSpacingAndDashes()
    {
        ...
        WriteLine("Transformed (lower case, then sentece case): {0}", ugly.Humanize().Transform(To.LowerCase, To.SentenceCase));
    }
    ...
    private static void OutputEnumNames()
    {
        ...
        WriteLine("Truncated to 8 charactes: {0}", favoriteAncientWonder.ToString().Truncate(length: 8));
    ...
    private static void NumberFormatting()
    {
        ...
        WriteLine();
        string[] things = { "fox", "person", "sheep", "apple", "goose",
        "oasis", "potato", "die", "dwarf", "attorney general", "biceps" };
        for (int i = 1; i <= 3; i++) 
        {
    ...
    private static void DateTimeFormatting()
    {
        ...
        foreach (int day in daysArray)
        {
            WriteLine("{0} days, Humanized: {1}", day, TimeSpan.FromDays(day).Humanize());
            WriteLine("{0} days, Humanized with precision 2: {1}", day, TimeSpan.FromDays(day).Humanize(precision: 2));
            WriteLine("{0} days, Humanized with max unit days: {1}", day, TimeSpan.FromDays(day).Humanize(maxUnit: Humanizer.Localisation.TimeUnit.Day));
            WriteLine();
        }
